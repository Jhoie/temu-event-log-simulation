...
def write_chunk_to_s3_parquet(df: pd.DataFrame, s3_path: str, run_id: str, 
                       chunk_counter: int):
    """
    ...
    try:
      ...
      logger.info(f"Writing chunk to file: {parquet_path}. Chunk size: {len(df)}")
      return parquet_path
      ...
      logger.error(f"An error occurred while writing chunk {chunk_counter} to parquet. {e}")
      raise
...
def split_events_into_chunks(config: dict, num_events: int, s3_path: str):
    ...
    try:
      ...
      run_id = datetime.now(ZoneInfo("Africa/Lagos")).strftime("%Y%m%dT%H%M%S")
      ...
      while remaining > 0:
          ...
          parquet_path = write_chunk_to_s3_parquet(df_chunk, s3_path, run_id, chunk_counter)

          remaining -= current_chunk_size
          logger.info(
            f"Chunk {chunk_counter:03d} of run_id {run_id} completed."
            f" This chunk generated {len(df_chunk)} rows(events) in {parquet_path} and"
            f" has {remaining} left"
            )
      ...
      logger.error(f"An error occurred while splitting events into chunks. {e}")
      raise
