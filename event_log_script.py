...
def generate_and_upload_events(num_events: int, json_path: str, s3_path: str):
    ...
    country = set(config["country"])
    platform_type = set(config["platform_type"])
    number_prefix_opt = set(config["number_prefix_opt"])
    events = set(config["all_events"])
    total_amount_events = set(config["total_amount_events"])
    payment_method_events = set(config["payment_method_events"])
    payment_method_type = config["payment_method_type"]
    payment_status_captured_events = set(config["payment_status_captured_events"])
    payment_status_refunded_events = set(config["payment_status_refunded_events"])
    carrier_events = set(config["carrier_events"])
    carrier_type = set(config["carrier_type"])
